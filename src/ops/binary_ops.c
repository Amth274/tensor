...
Tensor* tensor_sub(Tensor* t1, Tensor* t2)
{
    ...
    if (graph_add_node(g, OP_SUB, inputs, 2, outputs, 1) == INVALID_ID) {
        free(out);
        return NULL;
    }
...
Tensor* tensor_div(Tensor* t1, Tensor* t2)
{
    ...
    if (graph_add_node(g, OP_DIV, inputs, 2, outputs, 1) == INVALID_ID) {
        free(out);
        return NULL;
    }
...
Tensor* tensor_mat_mul(Tensor* t1, Tensor* t2)
{
    ...
    if (m1->dtype != m2->dtype) {
        fprintf(stderr, "ERROR: tensor_mat_mul requires matching dtypes for now\n");
        return NULL;
    }

    if (m1->ndims != 2 || m2->ndims != 2) {
        fprintf(stderr, "ERROR: tensor_mat_mul requires 2D tensors for now\n");
        return NULL;
    }

    if (m1->shape[1] != m2->shape[0]) {
        fprintf(stderr, "ERROR: tensor_mat_mul shape mismatch\n");
        return NULL;
    }

    int64_t out_shape[2] = {m1->shape[0], m2->shape[1]};
    uint8_t requires_grad = m1->requires_grad || m2->requires_grad;
    Tensor* out = tensor_create(g, 2, out_shape, m1->dtype, requires_grad);
    if (!out)
        return NULL;
    ...
    if (graph_add_node(g, OP_MATMUL, inputs, 2, outputs, 1) == INVALID_ID) {
        free(out);
        return NULL;
    }
